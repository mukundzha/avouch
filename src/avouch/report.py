...
def _render_premium(issues_by_file, file_reports, error_count, warning_count, suppressed, total_files):
    ...
    if passing_files:
        ...
        grid.add_column(style=f"dim {_SUCCESS}", no_wrap=True, max_width=26)
        grid.add_column(style=f"dim {_SUCCESS}", no_wrap=True, max_width=26)
        grid.add_column(style=f"dim {_SUCCESS}", no_wrap=True, max_width=26)
        limit = 30
        shown = passing_files[:limit]
        for i in range(0, len(shown), 3):
            row = shown[i:i+3]
            cells = [f"✓ {n}" for n in row]
            while len(cells) < 3:
                cells.append("")
            grid.add_row(*cells)
        if len(passing_files) > limit:
            grid.add_row("", f"[+{len(passing_files)-limit} more]", "", style=f"dim {_MUTED}")
        console.print(Panel(grid, border_style="#6b6560", padding=(0,2), title=f"passed  ·  {len(passing_files)}", title_align="left", expand=True, width=_LINE_WIDTH, box=box.SQUARE))
    console.print()
